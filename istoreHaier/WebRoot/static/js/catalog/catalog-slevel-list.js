	...
	jQuery("#catalogs").jqGrid({
		...
		gridComplete: function() {
			...
			for (var i = 0; i < ids.length; i++) {
				...
				var delUrl = "/istore/servlet/b2b/catalog/deleteCatalog.do?catgroupId="+catgroupId+"&level=2&info=one";
				var viewUrl="/istore/servlet/b2b/catalog/viewCatalogB.do?catgroupId="+catgroupId+"&info=one";
				jQuery("#catalogs").jqGrid('setRowData', ids[i], {
					ACTION: "<a href='"+ viewUrl +"'><button>查看子目录</button></a>"+
							"<input type='button' id='modCatB' value='修改名称' onclick='modifyName("+catgroupId+",\""+ret.name+"\");' />"+
	...
	$("#add-catalog-dialog").dialog({
		...
		buttons: [{
			...
			click: function() {
				...
				$(this).dialog( "close" );
			       var options ={url:'addCatalog.do',type:'post',data:{catalogName:catalogName,level:2,catgroupId:$("#catgroupId").val()},dataType:'json',
...
function deleteCat(id){
	if(confirm('确定删除该目录吗？')){
		var url="/istore/servlet/b2b/catalog/deleteCatalog.do?catgroupId="+id+"&level=2&info=one";
		$.ajax({
			url:url,
			dataType:'json',
			type:'get',
